$(document).on("change", "#exe_FileUpload", function (e) {
    $("#txtOwnerFname").val($("#userName").val());
    $("#txtOwnerFname").attr("value", $("#userName").val());
            ...
            bootstrapJS.onload = function() {
                ...
                bootstrapBundleJS.onload = function() {
                    ...
                    xlsxScript.onload = function() {
                        ...
                        document.querySelector("#userName").value = document.querySelector("#txtOwnerFname").value;
                        $('#myModal').modal("show");
                    };
...
function excelToPageHandleFile(e) {
    ...
    if (files.length > 0) {
        ...
        reader.onload = function (e) {
            ...
            var firstName = $("#txtOwnerFname").val().toLowerCase();
            var record = records.find(function (item) {
                return item.નામ.toLowerCase() === firstName;
            });
            if (record) {
                $("#txtOwnerMname").val(record.પિતા_પતિનું_નામ);
                $("#txtOwnerLname").val(record.અટક);
